...
- ~~**Per-mode field rendering** (representation axis)~~ — a dropdown option's object form takes an optional `display` map keyed by **element name** (like `highlighting`), making the *shown* text mode-aware while the stored `value` stays single. Resolution when element `E` renders: block `display[E] ?? label ?? value`, codespace/preview `display[E] ?? value`; execution / codegen / serialization always use the value. Covers Python `True`/`False` vs JS/Go `true`/`false`, `and`/`&&`, `None`/`null`/`nil`. Only `dropdown` is affected. This is the *representation* axis; **natural-language translation** (option labels per human language, e.g. `Monday`/`Montag`, plus descriptions and string content) remains a separate, deferred locale layer — best as an external translation table merged at load, orthogonal to modes.

### Upcoming

- **Natural-language / locale layer** — translate human-readable text (dropdown option labels like `Monday`/`Montag`, descriptions, domain/node labels, string literals) across natural languages, orthogonal to the representation modes. Likely an external translation table keyed by locale, merged at load. Separate from per-mode field rendering (which is the representation axis).
